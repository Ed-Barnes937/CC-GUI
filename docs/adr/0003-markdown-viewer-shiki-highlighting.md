# ADR-0003: Shiki for markdown viewer code blocks
...
package from the same pre-1.0 ecosystem (see ADR-0001) and no wiring to our
theme system.
...
- Post-processing the DOM is less elegant than a render-time adapter, but it
  is one contained function and keeps the renderer swappable (ADR-0001).
